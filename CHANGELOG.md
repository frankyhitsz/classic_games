...
- 统一 progress component hash 比较；数据库增加重置与存档删除屏障，旧 pending 不再跨越已提交删除。
- 存档隔离比较加载时的 hash、规则和版本；2048 不兼容格式保留，推箱子练习等待明确保存确认。
- 推箱子返回点改用紧凑指令历史，保留 v1 reader；恢复后的移动和撤销异步更新返回点。
- reject 标记在文件名对应锁内读取；恢复锁等待遵守扫描预算；过渡期兼容旧版成绩文件锁。
- 导出和 v2/v3 升级在发布前深度验证；发布不再使用硬链接；覆盖恢复增加预览和计划指纹。
- 导入业务终态与清理结果分离；准备目录先迁出再清理；证据读取统一拒绝 Windows reparse。
- CI 使用三平台 wheel hash 锁，增加服务契约与文件操作的类型检查。
