...
- CI #20 的存储、压力检查以及 Ubuntu/macOS gameplay 均通过；Windows coverage 下两个 HTTP
  正确性用例在产品客户端 0.7 秒读预算内未完成，结果为 105/107。用例现使用独立 5 秒预算和
  UUID 身份，产品超时未改；本机完整入口再次通过，并额外重复运行两个用例 20 次全部通过。
  CI runner 的失败注解也会保留每个失败后的断言上下文。
- CI #21 确认 Windows gameplay（含上述两个 HTTP 用例）已通过，同时给出两个新的明确证据：
  Windows coverage 下同步 SQLite p95 为 381 ms，macOS 的最近游戏用例在固定 0.4 秒退出前未
  收到异步结果。前者使用 Windows 专用 750 ms 落盘预算，其他 CI 保持 250 ms，游戏线程异步
  入队门槛仍为 10 ms；后者修正了“HTTP 模式却替换本地 backend”的测试隔离错误并改成 5 秒
  内条件等待。修复后的启动器单项独立重复 20 次通过。
- CI #22 的 core-only、Python 3.12/3.13 兼容检查以及 Ubuntu、macOS、Windows 三平台完整矩阵
  共 6 个作业全部通过，无失败注解。
