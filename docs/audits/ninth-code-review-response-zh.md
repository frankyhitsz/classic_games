...
- “最近游戏”用例原先启用 HTTP 模式却替换了本地 backend，实际依赖前一项测试写入的数据；
  现在替换真正被启动器导入的 HTTP backend，并按结果条件等待，单项和乱序执行不再漂移。
- Windows CI 的同步 SQLite fsync p95 单独限制为 750 ms，其他 CI 仍为 250 ms；游戏线程测量的
  异步入队 p99 仍为 10 ms，避免把共享 runner 磁盘抖动误当作输入卡顿或取消性能约束。
